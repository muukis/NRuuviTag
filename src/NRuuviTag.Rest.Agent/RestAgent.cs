...
public class RestAgent : RuuviTagPublisher {
    ...
    protected override async Task RunAsync(IAsyncEnumerable<RuuviTagSample> samples,
        CancellationToken cancellationToken) {
        Logger.LogInformation(Resources.LogMessage_RestClientStarting);

        var options = new RestClientOptions { ThrowOnAnyError = true, MaxTimeout = 10000 };
        ...
        async Task PublishBatch(bool timerElapsedEvent) {
            try {
                ...
                Logger.LogInformation(Resources.LogMessage_RestPublishEndSend);

                if (response?.StatusCode is not HttpStatusCode.OK) {
                    string GetStatusCodeText(HttpStatusCode? statusCode) =>
                        !statusCode.HasValue ? "NULL" : $"{(int) statusCode} ({statusCode})";

                    throw new HttpRequestException(
                        $"Invalid endpoint response: {GetStatusCodeText(response?.StatusCode)}");
                }
            }
            catch (Exception e) {
        ...
        try {
            ...
            await foreach (var sample in samples.ConfigureAwait(false)) {
                ...
                avgService.AddSample(RuuviTagSampleExtended.Create(sample, knownDevice?.DeviceId,
